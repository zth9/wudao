...
`invoke_claude_code_runner` 是 Agentic Chat 的一个工具，但 Claude Code Runner 本身是长耗时执行单元。它不能依赖前端页面、SSE 订阅或当前 React store 是否存在；页面刷新、抽屉关闭、事件流断开都只应影响展示，不应改变 Runner 的真实生命周期。

## 目标

- Claude Code Runner 的真实生命周期以 `task_sdk_runs` / `task_sdk_events` 为准
...
- 不把 Claude Code Runner 改成独立系统守护进程
...
- 服务端测试覆盖：调用 `invoke_claude_code_runner` 时会写入 SDK wait checkpoint，完成后清理
