...
- 回看结构化对话、Runner 历史、终端会话与任务状态
- 通过用户记忆和 Wudao Agent 记忆保留长期上下文

## 当前能力

- **任务中心**：支持任务解析、创建、筛选、分页、排序、优先级 P0-P4、截止时间、完成归档与统计摘要
- **Agentic Chat**：基于结构化 run/message 表、typed SSE 与工具协议，支持文本、工具调用、工具结果和错误
- **任务工作台**：左侧是 Agentic Chat，右侧是按任务记忆布局的终端与 Agent Runner 独立抽屉
- **Agent 工具**：支持 workspace 读写与 patch、workspace 搜索、终端快照、`invoke_claude_code_runner`
...
- **本地记忆**：用户记忆与 Wudao Agent 全局记忆保存在本机 profile 目录，并注入任务解析和聊天上下文
