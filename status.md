...
- **Claude Code Runner 已补独立等待状态与取消隔离**：修复 Agent Runtime 在收到 `invoke_claude_code_runner` 启动进度后误取消仍在等待完成的工具任务的问题；现在只取消临时进度队列等待，不会取消 Runner 工具任务本身；Agent run 等待 Runner 时会把 `sdk_run_id` 写入 `checkpoint_json`，刷新后即使实时 tool_call 进度丢失，也能从 SDK Runner 自身状态修复孤儿工具调用；已补充服务端回归测试覆盖 checkpoint 写入、完成清理与 checkpoint repair
- **Agentic Chat 已补 Debug 级运行时日志**：设置 `WUDAO_LOG_LEVEL=DEBUG` 后，服务端会用统一的 `agent_runtime.<event>` 前缀输出用户 prompt 摘要、每轮 AgentLoop 元数据、模型调用元数据、模型输出 preview、解析后的助手文本 / `tool_calls` 摘要、协议纠错、工具输入/进度/结果摘要以及 run 终态；不打印完整 message 历史；文本 preview 默认最多 800 字符，可用 `WUDAO_AGENT_DEBUG_TEXT_PREVIEW_CHARS` 调整，单条 JSON 日志默认最多 20000 字符，可用 `WUDAO_AGENT_DEBUG_LOG_MAX_CHARS` 调整
- **Agentic Chat 工具协议已补通用纠错**：AgentLoop 仍把 Agent Runner 视为普通工具，不做关键词或工具名特判；当模型在工具可用时输出非 JSON 文本、只口头承诺“马上调用 / 会执行”而没有给出 `tool_calls`，后端会追加一次统一协议纠错请求，要求模型把“调用任何可用工具”的意图改写为标准 `tool_calls`；已补充服务端回归测试覆盖非结构化工具承诺被纠正为普通工具调用
- **Agent Runner 成功结果已拆成助手消息气泡**：`invoke_claude_code_runner` / SDK Runner 成功返回 `final_text` 时，后端会把工具结果持久化为带 `final_text_split: true` 的精简元数据，并额外追加一条助手文本消息；前端只跳过带该显式标记的 SDK `tool_result`，因此失败结果和旧历史完整结果仍会显示在工具卡片里；刷新时自动修复孤儿 Runner 工具调用的路径也复用同一拆分规则；已补充后端与前端回归测试覆盖成功拆分、失败保留、旧历史保留和 thread repair
...
- **顶层导航栏现已支持响应式收缩**：浏览器宽度低于 `lg` 断点时，原先居中的四栏 pill 导航会收起为 HeroUI Dropdown 下拉胶囊，避免挤压左侧 Logo 和右侧用户头像；桌面端继续保持 pill 胶囊 + `active-pill` 滑动动效，已完成 `pnpm --filter web exec tsc`、`pnpm --filter web test`（17 个测试文件 / 109 个用例）与 `pnpm --filter web build`
